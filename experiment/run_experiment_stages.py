...
- Phase 1: Data Cleaning        -> phase_1_data_cleaning.py
- Phase 2: Data Transformation  -> phase_2_data_transformation.py
- Phase 3: EDA (post-transform) -> phase_3_eda.py
...
Note: EDA (Phase 3) is performed after Data Transformation (Phase 2) because the
project analyzes aggregated user-level features rather than raw source files.
This matches the physical pipeline order: Cleaning → Transformation → EDA → Labeling → Splitting.
...
def run_phase_2(cfg: StageConfig) -> None:
    log_phase_description("2")  # Phase 2: Data Cleaning
...
def run_phase_3(cfg: StageConfig) -> None:
    log_phase_description("3")  # Phase 3: Data Transformation
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--combined-file", type=Path, default=Path("phase2/combined_user_metrics.csv"))
